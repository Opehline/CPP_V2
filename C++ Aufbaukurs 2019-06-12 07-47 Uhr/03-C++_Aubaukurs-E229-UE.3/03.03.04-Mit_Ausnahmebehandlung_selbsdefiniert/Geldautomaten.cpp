	...
	if(u_evaluation>=0 && u_evaluation<=9)
	{
		...
		{	
			...
			default:
				...
				throw Ausnahmebehandlung(AUSNAHME_A);	// * Selbstdefinierter Rueckgabewert
		}
	}
	else
	{	
		std::cout << "\n**** Ausnahme 2 wurde hervorgerufen fuer Automaten Id : "<< u_id << "! *******" << std::endl;
		throw Ausnahmebehandlung(AUSNAHME_B); 	// * Selbstdefinierter Rueckgabewert
	}
}

void Geldautomaten::NetzTrennung(int id)
{	
	std::cout << "Geldautomaten Id. " 
			  << id 
...
void Geldautomaten::pubic_SystemAnbindungsProzess(int sa_id, int sa_evaluation)
{
